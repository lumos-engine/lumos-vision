...
def test_apply_color_cal_does_not_persist_cal_mode_locks(monkeypatch, tmp_path):
    from processor.app import Processor
    from processor.utils.color_calibrate import CalibrationSolution
    import numpy as np
    ...
    config = Config.from_dict(
        {
            ...
            "output": {"width": 320, "height": 180, "fps": 30, "v4l2": {"enabled": False}},
            "logging": {"stats_interval": 0},
            "lumos_cam": {"enabled": True, "af": "auto", "ae": "auto", "awb": "auto"},
            "boundary": {
                "mode": "manual",
                "corners": [[0.1, 0.1], [0.9, 0.1], [0.9, 0.9], [0.1, 0.9]],
            },
    ...
    app.start()
    try:
        app._color_cal.solution = CalibrationSolution(
            gains_bgr=(1.0, 1.0, 1.0),
            gamma=1.0,
            matrix_bgr=np.eye(3, dtype=np.float64),
        )
        app._color_cal.state = "ready"
        result = app.apply_color_calibration()
        assert result["ok"] is True
        assert app.config.lumos_cam.af == "auto"
        assert app.config.lumos_cam.ae == "auto"
        assert app.config.lumos_cam.awb == "auto"
    finally:
        app.shutdown()
