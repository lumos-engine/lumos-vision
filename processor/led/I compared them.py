I compared them. The result is better than I expected: we should use alphaonex86/openwrt as our main experimental base, but borrow RTL9607C board/platform pieces from jameywine/rtl9607c-dev where alpha's tree is incomplete.

Why alphaonex86 is now the more interesting base

The Anime4000 discussion is current and real: alphaonex86 reports RTL9607C Ethernet working, RTL9602C Ethernet/Wi-Fi/GPON work, and later RTL9607F with GPON + Ethernet + hardware offload + Wi-Fi. He also explicitly says different manufacturers require board-specific DTS work.

More importantly, his repository has moved well beyond a toy port. The current tree contains a dedicated realtek-luna target with rtl9607x and rtl960x subtargets. Recent work has a family GPON layer serving RTL9602C, RTL9603CVD and RTL9607C, plus a separate rtl9607c_gpon.o implementation.

His RTL8192FE work is particularly useful to us. On an RTL9602C board he got an RTL8192FE AP actually radiating, after fixing the card-enable power sequence and RF path initialization. Your 2.4-GHz device is exactly the RTL8192F/FE family, PCI ID 10ec:818c.

However, alpha's generic RTL9607C profile is still explicitly labelled a headless M1 bring-up target. His image definition says the RTL9607C target currently gets serial console + initramfs shell, with the full router/GPON package set intended as later work. So there is still no OP2200H-ready image to simply flash.

Meanwhile, jameywine's branch is useful because it is specifically focused on RTL9607C, has the current upstream OpenWrt PR, SPI-NAND/ECC support, PCIe bring-up, a very similar BT-PON RTL9607C board, and the exact PCI radio IDs we've found on related hardware. Its current device definition is still essentially an initramfs-oriented BT-G711AX target rather than an OP2200H install image.

So I would structure our fork as:

alphaonex86/openwrt main
        │
        ├── GPON / OMCI family work
        ├── Ethernet datapath work
        ├── RTL8192FE Wi-Fi work
        ├── HW-offload research
        │
        └── OP2200H board port
               │
               ├── borrow RTL9607C PCIe/NAND/DTS work
               │   from jameywine/rtl9607c-dev
               │
               ├── RTL8812F work from jameywine/rtw88 experiments
               │
               └── exact OP2200H GPIO/flash/radio configuration

Do not start from Anime4000/RTL960x itself. That repository is primarily the RTL960x reverse-engineering/community knowledge base. Alpha's actual OpenWrt fork is where the firmware implementation is happening.

Context primer for a new chat
...
One important change from our earlier plan: the 2.4-GHz side no longer looks like the biggest unknown. Alpha has already done substantial RTL8192FE bring-up and documented a real successful AP result. The more uncertain radio for our exact combination is now RTL8812F, where jameywine's experimental rtw88 work becomes particularly valuable.

So I would start the new chat from alphaonex86 main, not jameywine/rtl9607c-dev, while explicitly treating jameywine as our RTL9607C/PCIe/8812F donor/reference.